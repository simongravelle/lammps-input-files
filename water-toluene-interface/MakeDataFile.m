...
PnmpA=load('toluene/Positions.dat');
...
BnmpA=load('toluene/Bonds.dat');
BnmpA(:,2)=BnmpA(:,2)+1; % shift OH water
AnmpA=load('toluene/Angles.dat');
AnmpA(:,2)=AnmpA(:,2)+1; % shift HOH water
DnmpA=load('toluene/Dihedrals.dat');
DnmpA(:,2)=DnmpA(:,2);
InmpA=load('toluene/Impropers.dat');
InmpA(:,2)=InmpA(:,2);
